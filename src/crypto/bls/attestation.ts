...
export class G2Point extends mcl.G2 {
    constructor(xa: bigint, xb: bigint, ya: bigint, yb: bigint) {
        ...
        if (xa != undefined) {
            ...
            if (xa === 0n && xb === 0n && ya === 0n && yb === 0n) this.clear();
        }
    }
...
export class Signature extends G1Point {
    ...
    fromJson(s: { x: string; y: string }): Signature {
        return new Signature(BigInt(s.x), BigInt(s.y));
    }
...
export class PrivateKey extends mcl.Fr {
    constructor(secret?: string, base: number = 16) {
        super();
        if (secret) this.setStr(secret, base);
        else this.setStr(`${bn254Utils.random()}`, 10);
    }
...
export class KeyPair {
    ...
    constructor(privKey?: PrivateKey) {
        ...
        this.pubG1 = G1Point.fromStr(
            bn254Utils.mulByGeneratorG1(this.privKey).getStr(),
        );
        this.pubG2 = G2Point.fromStr(
            bn254Utils.mulByGeneratorG2(this.privKey).getStr(),
        );
    }
    ...
    async saveToFile(path: string, password: string) {
        ...
        // }
        let keystoreJson = await Web3Eth.accounts.encrypt(
            wallet.privateKey,
            password,
        );
        // @ts-ignore
    ...
    static async readFromFile(
        ...
        if (!keystoreJson.version) keystoreJson.version = 3;

        let keystoreAccount = await Web3Eth.accounts.decrypt(
            keystoreJson,
            password,
        );
...
export function newKeyPairFromString(
    secret: string,
    base: number = 16,
): KeyPair {
