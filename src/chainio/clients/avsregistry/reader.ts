...
export class AvsRegistryReader {
    ...
    async getOperatorsStakeInQuorumsAtCurrentBlock(quorumNumbers: QuorumNum[]): Promise<OperatorStateRetrieverOperator[][]> {
        const curBlock: bigint = await this.ethHttpClient.eth.getBlockNumber();
        return await this.getOperatorsStakeInQuorumsAtBlock(quorumNumbers, curBlock);
    }

    async getOperatorsStakeInQuorumsAtBlock(quorumNumbers: QuorumNum[], blockNumber: BlockNumber): Promise<OperatorStateRetrieverOperator[][]> {
        const quorumBytes = chainioUtils.numsToBytes(quorumNumbers.map(Number));
        const operatorStakes = await this.operatorStateRetriever.methods["getOperatorState(address,bytes,uint32)"](
    ...
    async getOperatorAddrsInQuorumsAtCurrentBlock(quorumNumbers: QuorumNum[]): Promise<string[][]> {
        const curBlock = await this.ethHttpClient.eth.getBlockNumber();
        const stakes = await this.getOperatorsStakeInQuorumsAtBlock(quorumNumbers, curBlock);
    ...
    async getOperatorsStakeInQuorumsOfOperatorAtCurrentBlock(operatorId: string): Promise<[number[], OperatorStateRetrieverOperator[][]]> {
        const curBlock = await this.ethHttpClient.eth.getBlockNumber();
        return await this.getOperatorsStakeInQuorumsOfOperatorAtBlock(operatorId, curBlock);
    }

    async getOperatorStakeInQuorumsOfOperatorAtCurrentBlock(operatorId: string): Promise<{ [key: number]: number }> {
        const quorumBitmap:bigint = await this.registryCoordinator.methods.getCurrentQuorumBitmap(operatorId).call();
        const quorums = chainioUtils.bitmapToQuorumIds(quorumBitmap);
        const result: { [key: number]: number } = {};
        for (const quorum of quorums) {
            result[quorum] = await this.stakeRegistry.methods.getCurrentStake(operatorId, quorum).call();
        }
        return result;
    }

    async weightOfOperatorForQuorum(quorumNumber: QuorumNum, operatorAddr: string): Promise<Uint96> {
        return await this.stakeRegistry.methods.weightOfOperatorForQuorum(quorumNumber, operatorAddr).call();
    }

    async strategyParamsLength(quorumNumber: QuorumNum): Promise<Uint256> {
        return await this.stakeRegistry.methods.strategyParamsLength(quorumNumber).call();
    }
    ...
    async getStakeHistoryLength(operatorId: string, quorumNumber: QuorumNum): Promise<Uint256> {
        return await this.stakeRegistry.methods.getStakeHistoryLength(operatorId, quorumNumber).call();
    }

    async getStakeHistory(operatorId: string, quorumNumber: QuorumNum): Promise<StakeUpdate[]> {
        const history: [Uint32, Uint32, Uint96][] = await this.stakeRegistry.methods.getStakeHistory(operatorId, quorumNumber).call();
    ...
    async getLatestStakeUpdate(operatorId: string, quorumNumber: QuorumNum): Promise<StakeUpdate> {
        const update = await this.stakeRegistry.methods.getLatestStakeUpdate(operatorId, quorumNumber).call();
        return {
            updateBlockNumber: update[0],
            nextUpdateBlockNumber: update[1],
            stake: update[2],
    ...
    async getStakeAtBlockNumber(operatorId: string, quorumNumber: QuorumNum, blockNumber: BlockNumber): Promise<Uint96> {
        return await this.stakeRegistry.methods.getStakeAtBlockNumber(operatorId, quorumNumber, blockNumber).call();
    }

    async getStakeUpdateIndexAtBlockNumber(operatorId: string, quorumNumber: QuorumNum, blockNumber: BlockNumber): Promise<Uint32> {
        return await this.stakeRegistry.methods.getStakeUpdateIndexAtBlockNumber(operatorId, quorumNumber, blockNumber).call();
    }

    async getTotalStakeHistoryLength(quorumNumber: QuorumNum): Promise<Uint256> {
        return await this.stakeRegistry.methods.getTotalStakeHistoryLength(quorumNumber).call();
    }
    ...
    async getCheckSignaturesIndices(
        ...
        const quorumBytes = chainioUtils.numsToBytes(quorumNumbers.map(Number));
        const result = await this.operatorStateRetriever.methods.getCheckSignaturesIndices(
            this.registryCoordinatorAddr,
    ...
    async getCurrentTotalStake(quorumNumber: QuorumNum): Promise<Uint96> {
        return await this.stakeRegistry.methods.getCurrentTotalStake(quorumNumber).call();
    }
    ...
    async getTotalStakeAtBlockNumberFromIndex(quorumNumber: QuorumNum, blockNumber: BlockNumber, index: Uint256): Promise<Uint96> {
        return await this.stakeRegistry.methods.getTotalStakeAtBlockNumberFromIndex(quorumNumber, blockNumber, index).call();
    }

    async getTotalStakeIndicesAtBlockNumber(quorumNumbers: QuorumNum[], blockNumber: BlockNumber): Promise<Uint32[]> {
        const quorumBytes = chainioUtils.numsToBytes(quorumNumbers.map(Number));
        return await this.stakeRegistry.methods.getTotalStakeIndicesAtBlockNumber(blockNumber, quorumBytes).call();
    }

    async getMinimumStakeForQuorum(quorumNumber: QuorumNum): Promise<Uint96> {
        return await this.stakeRegistry.methods.minimumStakeForQuorum(quorumNumber).call();
    }

    async getStrategyParamsAtIndex(quorumNumber: QuorumNum, index: Uint256): Promise<StrategyParams> {
        return await this.stakeRegistry.methods.strategyParams(quorumNumber, index).call();
    }

    async getStrategyPerQuorumAtIndex(quorumNumber: QuorumNum, index: Uint256): Promise<string> {
        return await this.stakeRegistry.methods.strategiesPerQuorum(quorumNumber, index).call();
    }

    async getRestakeableStrategies(): Promise<string[]> {
        return await this.serviceManager.methods.getRestakeableStrategies().call();
    }

    async getOperatorRestakedStrategies(operator: string): Promise<string[]> {
        return await this.serviceManager.methods.getOperatorRestakedStrategies(operator).call();
    }

    async getStakeTypePerQuorum(quorumNumber: QuorumNum): Promise<Uint8> {
        return await this.stakeRegistry.methods.stakeTypePerQuorum(quorumNumber).call();
    }

    async getSlashableStakeLookAheadPerQuorum(quorumNumber: QuorumNum): Promise<Uint32> {
        return await this.stakeRegistry.methods.slashableStakeLookAheadPerQuorum(quorumNumber).call();
    }
    ...
    async getOperatorFromId(operatorId: string): Promise<string> {
        return await this.registryCoordinator.methods.getOperatorFromId(operatorId).call();
    }

    async queryRegistrationDetail(operatorAddress: Address): Promise<boolean[]> {
        const operatorId = await this.getOperatorId(operatorAddress);
        const value: Uint192 = await this.registryCoordinator.methods.getCurrentQuorumBitmap(operatorId).call();
        const bits: boolean[] = [];
    ...
    async getOperatorIdFromOperatorAddress(operatorAddress: string): Promise<string> {
        return await this.blsApkRegistry.methods.operatorToPubkeyHash(operatorAddress).call();
    }

    async getOperatorAddressFromOperatorId(operatorPubkeyHash: string): Promise<string> {
        return await this.blsApkRegistry.methods.pubkeyHashToOperator(operatorPubkeyHash).call();
    }
    ...
    async queryExistingRegisteredOperatorSockets(
        ...
        const eventAbi = this.registryCoordinator.events.OperatorSocketUpdate().abi;
        // const eventTopic = ethers.id('OperatorSocketUpdate(bytes32,string)');
        const eventTopic = this.ethHttpClient.eth.abi.encodeEventSignature(eventAbi);

        for (let i = startBlock; i <= stopBlock; i += blockRange) {
            const toBlock:BlockNumber = chainioUtils.min(i + blockRange - 1n, stopBlock);
            ...
            try {
                ...
                this.logger.debug('avsRegistryChainReader.queryExistingRegisteredOperatorSockets', {
                    numTransactionLogs: decodedLogs.length,
                    fromBlock: i,
                    toBlock: toBlock,
                });
            } catch (e) {
                this.logger.warn(`Failed to fetch logs for blocks ${i}-${toBlock}: ${e}`);
                continue;
            }
        }
    ...
    async queryExistingRegisteredOperatorPubkeys(
        ...
        for (let i = startBlock; i <= stopBlock; i += blockRange) {
            ...
            const eventAbi = this.blsApkRegistry.events.NewPubkeyRegistration().abi;
            ...
            this.logger.debug('avsRegistryChainReader.queryExistingRegisteredOperatorPubkeys', {
                numTransactionLogs: decodedLogs.length,
                fromBlock: i,
                toBlock: toBlock,
            });
    ...
    async canSatisfyOnlyCoordinatorOwnerModifier(address: string): Promise<boolean> {
