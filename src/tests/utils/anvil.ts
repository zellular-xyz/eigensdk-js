...
export const ANVIL_FIRST_PRIVATE_KEY = 'ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80';
export const ANVIL_SECOND_ADDRESS = '0x70997970C51812dc3A010C7d01b50e0d17dc79C8';
...
export async function startAnvilContainer(
    ...
    try {
        const container = new GenericContainer('ghcr.io/foundry-rs/foundry:stable')
            .withEntrypoint(['anvil'])
            .withCommand(['--host', '0.0.0.0', '--base-fee', '0', '--gas-price', '0'])
            // .withExposedPorts({ container: 8545, host: 8545 })
            .withExposedPorts(8545)
            .withWaitStrategy(Wait.forLogMessage('Listening on', 1));

        if (anvilStateFileName) {
            console.log(`Starting Anvil container with state file: ${anvilStateFileName}`);
            container.withCommand(['--host', '0.0.0.0', '--base-fee', '0', '--gas-price', '0', '--load-state', '/mnt/state.json']);

            // Resolve the state file path relative to the current file
            const __filename = fileURLToPath(import.meta.url);
            const __dirname = path.dirname(__filename);
            const stateFilePath = path.join(__dirname, '..', '..', isM2 ? 'M2-contracts/anvil' : 'contracts/anvil', anvilStateFileName);
...
export async function startM2AnvilContainer(anvilStateFileName: string = ''): Promise<{ container: any; endpoint: string }> {
...
export async function getContractAddressesFromContractRegistry(
    ...
    try {
        ...
        const contractsRegistry = new ethers.Contract(CONTRACTS_REGISTRY_ADDRESS, contractsRegistryAbi, provider);

        const mockAvsServiceManagerAddr = await contractsRegistry.contracts('mockAvsServiceManager');
        if (mockAvsServiceManagerAddr === ethers.ZeroAddress) {
            throw new Error('mockAvsServiceManagerAddr is empty');
        }

        const mockAvsRegistryCoordinatorAddr = await contractsRegistry.contracts('mockAvsRegistryCoordinator');
        if (mockAvsRegistryCoordinatorAddr === ethers.ZeroAddress) {
            throw new Error('mockAvsRegistryCoordinatorAddr is empty');
        }

        const mockAvsOperatorStateRetrieverAddr = await contractsRegistry.contracts('mockAvsOperatorStateRetriever');
        if (mockAvsOperatorStateRetrieverAddr === ethers.ZeroAddress) {
            throw new Error('mockAvsOperatorStateRetrieverAddr is empty');
        }

        const delegationManagerAddr = await contractsRegistry.contracts('delegationManager');
        if (delegationManagerAddr === ethers.ZeroAddress) {
            throw new Error('delegationManagerAddr is empty');
        }

        const erc20MockStrategyAddr = await contractsRegistry.contracts('erc20MockStrategy');
        if (erc20MockStrategyAddr === ethers.ZeroAddress) {
            throw new Error('erc20MockStrategyAddr is empty');
        }

        const rewardsCoordinatorAddr = await contractsRegistry.contracts('rewardsCoordinator');
        if (rewardsCoordinatorAddr === ethers.ZeroAddress) {
            throw new Error('rewardsCoordinatorAddr is empty');
        }

        const registryCoordinator = new ethers.Contract(mockAvsRegistryCoordinatorAddr, ABIs.REGISTRY_COORDINATOR_ABI, provider);
        ...
        if (!isM2) {
            const permissionControllerAddr = await contractsRegistry.contracts('permissionController');
...
export async function getM2ContractAddressesFromContractRegistry(ethHttpUrl: string): Promise<ContractAddresses> {
    return getContractAddressesFromContractRegistry(ethHttpUrl, true);
}

// Advance chain by N blocks using cast command
export async function advanceChainByNBlocks(n: number, anvilEndpoint: string): Promise<void> {
    try {
        const { stdout, stderr } = await execAsync(`cast rpc anvil_mine ${n} --rpc-url ${anvilEndpoint}`);
        if (stderr) {
            throw new Error(`Error in cast command: ${stderr}`);
        }
        console.log(stdout);
    } catch (error) {
        throw new Error(`Failed to advance chain by ${n} blocks: ${error.message}`);
    }
}

// Advance chain by N blocks inside the container
export async function advanceChainByNBlocksExecInContainer(n: number, container: any): Promise<void> {
    try {
        // @ts-ignore
        ...
        if (exitCode !== 0) {
            throw new Error(`Unable to advance anvil chain by ${n} blocks. Exit code: ${exitCode}, Output: ${output}`);
        }
    } catch (error: any) {
        throw new Error(`Failed to advance chain by ${n} blocks in container: ${error.message}`);
    }
}