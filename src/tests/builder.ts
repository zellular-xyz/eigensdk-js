...
async function buildClients(rpcEndpoint: string): Promise<{clients: Clients[], addresses: ContractAddresses}> {
    const addresses = await getContractAddressesFromContractRegistry(rpcEndpoint)
    ...
    for (let i = 0; i < 3; i++) {
        clients.push(await buildAll(allConfigs, config[`ecdsa_private_key_${i + 1}`]));
    }

    return {clients, addresses}
}

export async function buildSingleClient(privateKey: string, rpcEndpoint: string, addresses?: ContractAddresses): Promise<Clients> {
    if(!addresses) {
