...
describe.sequential("AvsRegistryWriter", () => {
    ...
    test("registerAsOperatorForOperatorSets", async () => {
        for (let i = 0; i < 3; i++) {
            ...
            const setReceipt = await clients[
                i
            ].elWriter.registerForOperatorSets(addresses.registryCoordinator, {
            ...
            logger.info(
                `Registered for operator sets with tx hash: ${setReceipt.transactionHash}`,
            );
        }
    });
    ...
    test("updateStakesOfOperatorSubsetForAllQuorums", async () => {
        ...
        const receipt =
            await client0.avsRegistryWriter.updateStakesOfOperatorSubsetForAllQuorums(
                operators,
            );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Updated stakes for operator subset with tx hash: ${receipt.transactionHash}`,
        );
    });

    test("setRewardsInitiator", async () => {
        const receipt = await client0.avsRegistryWriter.setRewardsInitiator(
            config.operator_address_1,
        );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Set rewards initiator with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("setMinimumStakeForQuorum", async () => {
        ...
        const receipt =
            await client0.avsRegistryWriter.setMinimumStakeForQuorum(
                quorumNumber,
                minimumStake,
            );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Set minimum stake for quorum with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("createTotalDelegatedStakeQuorum", async () => {
        ...
        const strategyParams: StrategyParams[] = [
            {
                strategy: addresses.erc20MockStrategy,
                multiplier: 10000n,
            },
        ];
        const receipt =
            await client0.avsRegistryWriter.createTotalDelegatedStakeQuorum(
                operatorSetParams,
        ...
        expect(receipt.status).toBe(1n);
        logger.info(
            `Created total delegated stake quorum with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("setOperatorSetParams", async () => {
        ...
        const receipt = await client0.avsRegistryWriter.setOperatorSetParams(
            quorumNumber,
            operatorSetParams,
        );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Set operator set params with tx hash: ${receipt.transactionHash}`,
        );
    });

    test("setChurnApprover", async () => {
        const receipt = await client0.avsRegistryWriter.setChurnApprover(
            config.operator_address_1,
        );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Set churn approver with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("setEjector", async () => {
        ...
        const receipt =
            await client0.avsRegistryWriter.setEjector(ejectorAddress);
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
    ...
    test("modifyStrategyParams", async () => {
        ...
        const receipt = await client0.avsRegistryWriter.modifyStrategyParams(
            quorumNumber,
            strategyIndices,
            multipliers,
        );
        ...
        expect(receipt.status).toBe(1n);
        logger.info(
            `Modified strategy parameters with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("setEjectionCooldown", async () => {
        ...
        const receipt =
            await client0.avsRegistryWriter.setEjectionCooldown(
                ejectionCooldown,
            );
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Set ejection cooldown with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("updateAvsMetadataUri", async () => {
        ...
        const receipt =
            await client0.avsRegistryWriter.updateAvsMetadataUri(metadataUri);
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Updated AVS metadata URI with tx hash: ${receipt.transactionHash}`,
        );
    });

    test("createAvsRewardsSubmission", async () => {
        const strategyParams0 =
            await client0.avsRegistryReader.strategyParamsByIndex(0n, 0n);
        ...
        const duration = Number(
            await client0.elReader.getCalculationIntervalSeconds(),
        );
        const [, token] = await client0.elReader.getStrategyAndUnderlyingToken(
            strategyParams0.strategy,
        );
        ...
        const receipt =
            await client0.avsRegistryWriter.createAvsRewardsSubmission([
                rewardsSubmission,
            ]);
        expect(receipt).not.toBeNull();
        expect(receipt.status).toBe(1n);
        logger.info(
            `Created AVS rewards submission with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("createSlashableStakeQuorum", async () => {
        ...
        const strategyParam: StrategyParams = {
        ...
        try {
            const receipt =
                await client0.avsRegistryWriter.createSlashableStakeQuorum(
                    operatorSetParams,
                    minimumStakeRequired,
                    [strategyParam],
                    lookAheadPeriod,
            ...
            expect(receipt).not.toBeNull();
            logger.info(
                `Created slashable stake quorum with tx hash: ${receipt.transactionHash}`,
            );
        } catch (e) {
            console.log(e);
            throw e;
    ...
    test("setSlashableStakeLookahead", async () => {
        ...
        const receipt2 =
            await client0.avsRegistryWriter.setSlashableStakeLookahead(
                newQuorium,
                newLookAheadPeriod,
            );
    ...
    test("addStrategies", async () => {
        const strategies =
            await client0.avsRegistryReader.strategyParamsByIndex(0n, 0n);
        expect(strategies).not.toBeNull();
        const strategyAddr = client0.ethHttpClient.utils.toChecksumAddress(
            strategies.strategy,
        );
        const strategyParams: StrategyParams = {
            strategy: strategyAddr,
        ...
        try {
            ...
            expect.fail(
                "addStrategies with exesting strategy must not execute successfully",
            );
        } catch (error) {
    ...
    test("ejectOperator", async () => {
        ...
        const receipt = await client0.avsRegistryWriter.ejectOperator(address, [
            0n,
        ]);
        expect(receipt).not.toBeNull();
        logger.info(
            `Ejected operator with tx hash: ${receipt.transactionHash}`,
        );
    });
    ...
    test("removeStrategies", async () => {
        try {
            ...
            const newStrategy: StrategyParams = {
                strategy: config.avs_address,
                multiplier: 1000n,
            };
            const addReceipt = await client0.avsRegistryWriter.addStrategies(
                0n,
                [newStrategy],
            );
            ...
            const receipt = await client0.avsRegistryWriter.removeStrategies(
                quorumNumber,
                indicesToRemove,
            );
            expect(receipt).not.toBeNull();
            logger.info(
                `Removed strategies with tx hash: ${receipt.transactionHash}`,
            );
        } catch (e: any) {
    ...
    test("createOperatorDirectedAvsRewardsSubmission", async () => {
        try {
            ...
            const [, token] =
                await client0.elReader.getStrategyAndUnderlyingToken(
                    strategyParams0.strategy,
                );
            expect(token).not.toBeNull();
            const latestBlock =
                await client0.ethHttpClient.eth.getBlock("latest");
            const blockTime = Number(latestBlock.timestamp);
            const startTimestamp =
                Math.floor(blockTime / duration - 2) * duration;
            const strategyParams: StrategyParams = {
                ...strategyParams0,
                multiplier: 1n,
            };
            const rewardsSubmission: OperatorDirectedRewardsSubmission = {
                ...
                token,
                operatorRewards: [
                    { operator: config.operator_address_1, amount: 1000n },
                ],
                startTimestamp: BigInt(startTimestamp),
                duration: BigInt(duration),
            ...
            const receipt =
                await client0.avsRegistryWriter.createOperatorDirectedAvsRewardsSubmission(
                    [rewardsSubmission],
                );
            expect(receipt).not.toBeNull();
            logger.info(
