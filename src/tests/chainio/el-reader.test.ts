...
describe('ELReader', () => {
    ...
    test('getAllocatableMagnitude', async () => {
        ...
        const result = await client0.elReader.getAllocatableMagnitude(operatorAddr, strategyAddr);
    ...
    test('getMaxMagnitudes', async () => {
        ...
        const result = await client0.elReader.getMaxMagnitudes(operatorAddr, strategyAddresses);
        expect(Array.isArray(result)).toBe(true);
        expect(result.every(magnitude => typeof magnitude === 'bigint')).toBe(true);
    ...
    test('getAllocationInfo', async () => {
        const operatorAddr = config.operator_address_1;
        const strategyAddr = addresses.erc20MockStrategy;
        const result = await client0.elReader.getAllocationInfo(operatorAddr, strategyAddr);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('getOperatorShares', async () => {
        const operatorAddr = config.operator_address_1;
        const strategyAddresses = [addresses.erc20MockStrategy];
        const result = await client0.elReader.getOperatorShares(operatorAddr, strategyAddresses);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('getOperatorSetsForOperator', async () => {
        const operatorAddr = config.operator_address_1;
        const result = await client0.elReader.getOperatorSetsForOperator(operatorAddr);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('isOperatorRegisteredWithAvs', async () => {
        ...
        const result = await client0.elReader.isOperatorRegisteredWithAvs(operatorAddr, avsAddr);
    ...
    test('isOperatorRegisteredWithOperatorSet', async () => {
        const operatorAddr = config.operator_address_1;
        const registeredSets = await client0.elReader.getRegisteredSets(operatorAddr);
        ...
        const result = await client0.elReader.isOperatorRegisteredWithOperatorSet(operatorAddr, operatorSet);
        expect(typeof result).toBe('boolean');
    ...
    test('isOperatorSlashable', async () => {
        const operatorAddr = config.operator_address_1;
        const operatorSets = await client0.elReader.getOperatorSetsForOperator(operatorAddr);
        const operatorSet: OperatorSet = operatorSets.length > 0
            ? operatorSets[0]
            : { id: 0n, avs: addresses.registryCoordinator };
        const result = await client0.elReader.isOperatorSlashable(operatorAddr, operatorSet);
        expect(typeof result).toBe('boolean');
    ...
    test('getAllocatedStake', async () => {
        ...
        const operatorSet: OperatorSet = operatorSets.length > 0
            ? operatorSets[0]
            : { id: 0n, avs: addresses.registryCoordinator };
        const result = await client0.elReader.getAllocatedStake(operatorSet, [operatorAddr], [strategyAddr]);
        expect(Array.isArray(result)).toBe(true);
        if (result.length > 0) {
            expect(result.every(innerList => Array.isArray(innerList))).toBe(true);
            for (const innerList of result) {
                expect(innerList.every(stake => typeof stake === 'bigint')).toBe(true);
            }
        }
    ...
    test('getOperatorsForOperatorSet', async () => {
        ...
        const result = await client0.elReader.getOperatorsForOperatorSet(operatorSet);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('getNumOperatorsForOperatorSet', async () => {
        ...
        const operatorSet: OperatorSet = operatorSets.length > 0
            ? operatorSets[0]
            : { id: 0n, avs: addresses.registryCoordinator };
        const result = await client0.elReader.getNumOperatorsForOperatorSet(operatorSet);
        expect(typeof result).toBe('bigint');
    ...
    test('getStrategiesForOperatorSet', async () => {
        const avsAddr = addresses.avsAddress;
        const operatorSet: OperatorSet = { id: 1n, avs: avsAddr };
        const result = await client0.elReader.getStrategiesForOperatorSet(operatorSet);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('isOperatorRegistered', async () => {
        const operatorAddr = config.operator_address_1;
        const result = await client0.elReader.isOperatorRegistered(operatorAddr);
        expect(typeof result).toBe('boolean');
    ...
    test('getDelegatedOperator', async () => {
        ...
        expect(client0.ethHttpClient.utils.isAddress(result) || result === '0x0000000000000000000000000000000000000000').toBe(true);
        logger.info(`Delegated operator: ${result}`);
        const currentBlock = await client0.ethHttpClient.eth.getBlockNumber();
        const resultWithBlock = await client0.elReader.getDelegatedOperator(stakerAddr, currentBlock);
        expect(client0.ethHttpClient.utils.isAddress(resultWithBlock) || resultWithBlock === '0x0000000000000000000000000000000000000000').toBe(true);
    ...
    test('getOperatorDetails', async () => {
        ...
        expectToHaveProps(result, ['address', 'delegationApproverAddress', 'allocationDelay']);
        expect(client0.ethHttpClient.utils.isAddress(result.address)).toBe(true);
    ...
    test('getOperatorSharesInStrategy', async () => {
        const operatorAddr = config.operator_address_1;
        const strategyAddr = addresses.erc20MockStrategy;
        const result = await client0.elReader.getOperatorSharesInStrategy(operatorAddr, strategyAddr);
        expect(typeof result).toBe('bigint');
    ...
    test('calculateDelegationApprovalDigestHash', async () => {
        ...
        const expiry = 2 ** 32 - 1;
        const result = await client0.elReader.calculateDelegationApprovalDigestHash(
            stakerAddr,
            operatorAddr,
            delegationApprover,
            approverSalt,
    ...
    test('getOperatorsShares', async () => {
        const operatorAddr = config.operator_address_1;
        const strategyAddr = addresses.erc20MockStrategy;
        const result = await client0.elReader.getOperatorsShares([operatorAddr], [strategyAddr]);
        expect(Array.isArray(result)).toBe(true);
        expect(result.every(innerList => Array.isArray(innerList))).toBe(true);
        for (const innerList of result) {
            expect(innerList.every(share => typeof share === 'bigint')).toBe(true);
        }
    ...
    test('getDelegationApproverSaltIsSpent', async () => {
        ...
        const approverSalt = chainIoUtils.numsToBytes(new Array(32).fill(0));
        const result = await client0.elReader.getDelegationApproverSaltIsSpent(delegationApprover, approverSalt);
        expect(typeof result).toBe('boolean');
    ...
    test('getPendingWithdrawalStatus', async () => {
        ...
        const result = await client0.elReader.getPendingWithdrawalStatus(withdrawalRoot);
        expect(typeof result).toBe('boolean');
    ...
    test('getCumulativeWithdrawalsQueued', async () => {
        const stakerAddr = config.operator_address_1;
        const result = await client0.elReader.getCumulativeWithdrawalsQueued(stakerAddr);
        expect(typeof result).toBe('bigint');
    ...
    test('canCall', async () => {
        ...
        const result = await client0.elReader.canCall(accountAddr, appointeeAddr, targetAddr, selector);
        expect(typeof result).toBe('boolean');
    ...
    test('listAppointees', async () => {
        ...
        const targetAddr = addresses.registryCoordinator;
        const selector = chainIoUtils.numsToBytes([12, 34, 56, 78]);
        const result = await client0.elReader.listAppointees(accountAddr, targetAddr, selector);
        expect(Array.isArray(result)).toBe(true);
    ...
    test('listAppointeePermissions', async () => {
        const accountAddr = config.operator_address_1;
        const appointeeAddr = config.operator_address_1;
        const [targets, selectors] = await client0.elReader.listAppointeePermissions(accountAddr, appointeeAddr);
        ...
        logger.info(`Appointee permissions: targets=${jsonEncode(targets)}, selectors=[${selectors.map(s => s).join(', ')}]`);
    });
    ...
    test('isPendingAdmin', async () => {
        ...
        const result = await client0.elReader.isPendingAdmin(accountAddr, pendingAdminAddr);
        expect(typeof result).toBe('boolean');
    ...
    test('getCurrRewardsCalculationEndTimestamp', async () => {
        const result = await client0.elReader.getCurrRewardsCalculationEndTimestamp();
        expect(typeof result).toBe('bigint');
        logger.info(`Current rewards calculation end timestamp: ${result}`);
    });

    test('getCurrentClaimableDistributionRoot', async () => {
        const result = await client0.elReader.getCurrentClaimableDistributionRoot();
        expectToHaveProps(result, ['root', 'rewardsCalculationEndTimestamp', 'activatedAt', 'disabled']);
        logger.info(`Current claimable distribution root: ${jsonEncode(result)}`);
    });

    test('getCumulativeClaimed', async () => {
        const earnerAddr = config.operator_address_1;
        const tokenAddr = addresses.erc20MockStrategy;
        const result = await client0.elReader.getCumulativeClaimed(earnerAddr, tokenAddr);
        expect(typeof result).toBe('bigint');
    ...
    test('getOperatorAvsSplit', async () => {
        const operatorAddr = config.operator_address_1;
        const avsAddr = addresses.avsAddress;
        const result = await client0.elReader.getOperatorAvsSplit(operatorAddr, avsAddr);
        expect(typeof result).toBe('bigint');
    ...
    test('getOperatorSetSplit', async () => {
        ...
        const operatorSet: OperatorSet = operatorSets.length > 0
            ? operatorSets[0]
            : { id: 0n, avs: addresses.registryCoordinator };
        const result = await client0.elReader.getOperatorSetSplit(operatorAddr, operatorSet);
        expect(typeof result).toBe('bigint');
    ...
    test('getIsAvsRewardsSubmissionHash', async () => {
        ...
        const result = await client0.elReader.getIsAvsRewardsSubmissionHash(avsAddr, hashValue);
        expect(typeof result).toBe('boolean');
    ...
    test('getIsRewardsSubmissionForAllHash', async () => {
        const avsAddr = addresses.avsAddress;
        const hashValue = chainIoUtils.numsToBytes(new Array(32).fill(0));
        const result = await client0.elReader.getIsRewardsSubmissionForAllHash(avsAddr, hashValue);
        expect(typeof result).toBe('boolean');
    ...
    test('getIsRewardsForAllSubmitter', async () => {
        ...
        const result = await client0.elReader.getIsRewardsForAllSubmitter(submitterAddr);
        expect(typeof result).toBe('boolean');
    ...
    test('getIsRewardsSubmissionForAllEarnersHash', async () => {
        const avsAddr = addresses.avsAddress;
        const hashValue = chainIoUtils.numsToBytes(new Array(32).fill(0));
        const result = await client0.elReader.getIsRewardsSubmissionForAllEarnersHash(avsAddr, hashValue);
        expect(typeof result).toBe('boolean');
    ...
    test('getIsOperatorDirectedAvsRewardsSubmissionHash', async () => {
        const avsAddr = addresses.avsAddress;
        const hashValue = chainIoUtils.numsToBytes(new Array(32).fill(0));
        const result = await client0.elReader.getIsOperatorDirectedAvsRewardsSubmissionHash(avsAddr, hashValue);
        expect(typeof result).toBe('boolean');
    ...
    test('getIsOperatorDirectedOperatorSetRewardsSubmissionHash', async () => {
        const avsAddr = addresses.avsAddress;
        const hashValue = chainIoUtils.numsToBytes(new Array(32).fill(0));
        const result = await client0.elReader.getIsOperatorDirectedOperatorSetRewardsSubmissionHash(avsAddr, hashValue);
        expect(typeof result).toBe('boolean');
    ...
    test('getStrategyAndUnderlyingToken', async () => {
        const strategyAddr = addresses.erc20MockStrategy;
        const [strategyContract, tokenAddr] = await client0.elReader.getStrategyAndUnderlyingToken(strategyAddr);
    ...
    test('getStrategyAndUnderlyingErc20Token', async () => {
        const strategyAddr = addresses.erc20MockStrategy;
        const [strategyContract, tokenContract, tokenAddr] = await client0.elReader.getStrategyAndUnderlyingErc20Token(strategyAddr);
        expect(client0.ethHttpClient.utils.isAddress(tokenAddr)).toBe(true);
    ...
    test('calculateOperatorAvsRegistrationDigestHash', async () => {
        ...
        const expiry = 2 ** 32 - 1;
        // @ts-ignore
        const result: string = await client0.elReader.calculateOperatorAvsRegistrationDigestHash(operatorAddr, avsAddr, salt, BigInt(expiry));
        expect(typeof result).toBe('string');
    ...
    test('getEncumberedMagnitude', async () => {
        const operatorAddr = config.operator_address_1;
        const strategyAddr = addresses.erc20MockStrategy;
        const result = await client0.elReader.getEncumberedMagnitude(operatorAddr, strategyAddr);
        expect(typeof result).toBe('bigint');
    ...
    test('getNumOperatorSetsForOperator', async () => {
        const operatorAddr = config.operator_address_1;
        const result = await client0.elReader.getNumOperatorSetsForOperator(operatorAddr);
    ...
    test('getSlashableShares', async () => {
        ...
        const operatorSet: OperatorSet = operatorSets.length > 0
            ? operatorSets[0]
            : { id: 0n, avs: addresses.registryCoordinator };
        const result = await client0.elReader.getSlashableShares(operatorAddr, operatorSet, [strategyAddr]);
    ...
    test('getSlashableSharesForOperatorSetsBefore', async () => {
        const operatorAddr = config.operator_address_1;
        let operatorSets = await client0.elReader.getOperatorSetsForOperator(operatorAddr);
        if (!operatorSets.length) {
            operatorSets = [{ id: 1n, avs: addresses.registryCoordinator }];
        }
        const currentBlock = await client0.ethHttpClient.eth.getBlockNumber();
        const result = await client0.elReader.getSlashableSharesForOperatorSetsBefore(operatorSets, currentBlock);
        expect(Array.isArray(result)).toBe(true);
        logger.info(`Slashable shares for operator sets before: ${jsonEncode(result)}`);
    });
    ...
    test('getSlashableSharesForOperatorSets', async () => {
        ...
        const result: SlashableStake[] | null = await client0.elReader.getSlashableSharesForOperatorSets(operatorSets);
        expect(Array.isArray(result)).toBe(true);
        for (const item of result || []) {
            ...
            expect(client0.ethHttpClient.utils.isAddress(operatorSet.avs)).toBe(true);
        ...
        logger.info(`Slashable shares for operator sets: ${jsonEncode(result)}`);
    });
});