...
import {
    ...
    sendContractCall,
} from "../../chainio/utils.js";
import {
    OperatorKickParam,
    RegistrationRequest,
    SignatureWithSaltAndExpiry,
} from "../../chainio/clients/elcontracts/types.js";
import { abiEncodeData } from "../../utils/helpers.js";
import {
    ClaimCheckParams,
    ELReader,
} from "../../chainio/clients/elcontracts/reader.js";
...
describe("elwriter tests", async () => {
    ...
    describe("RegisterOperator", () => {
        ...
        beforeAll(async () => {
            ...
            ({ container, endpoint } = await testUtils.startAnvilContainer(
        ...
        test("should register as an operator", async () => {
            ...
            const fundingClient = await buildSingleClient(
                fundedPrivateKeyHex,
                endpoint,
            );
            const receipt =
                await fundingClient.elWriter.registerAsOperator(operator);
        ...
        test("should fail to register an already registered operator", async () => {
            ...
            const operatorClient = await buildSingleClient(
    ...
    describe("RegisterAndDeregisterFromOperatorSets", async () => {
        const testConfigs = testUtils.getDefaultTestConfig();
        const { endpoint } = await testUtils.startAnvilContainer(
            testConfigs.anvilStateFileName,
        );
        ...
        const receipt = await createTotalStakeOperatorSet(
            client0,
    ...
    describe("RegisterOperatorSetWithChurn", async () => {
        ...
        test("should register operator set with churn", async () => {
            ...
            const receipt =
                await clients[0].avsRegistryWriter.setOperatorSetParams(
                    operatorSetId,
                    opsetParams,
                );
            expect(receipt.status).toBe(1n); // 1 indicates successful transaction in ethers.js
            ...
            registrationRequest.churnApprovalEcdsaPrivateKey =
            ...
            try {
                const receipt2 =
                    await op2Clients.elWriter.registerForOperatorSets(
                        addresses.registryCoordinator,
                        registrationRequest,
                    );
            ...
            const isOp1Registered =
                await clients[0].elReader.isOperatorRegisteredWithOperatorSet(
                    op1Address,
                    operatorSet,
                );
            ...
            const isOp2Registered =
                await op2Clients.elReader.isOperatorRegisteredWithOperatorSet(
                    op2Address,
                    operatorSet,
                );
    ...
    function getExampleRegistrationParams(): PubkeyRegistrationParams {
        return {
            ...
            pubkeyG1: [
                BigInt(
                    "10371454967541283327403832945957227913391851874635485454053224012738342927470",
                ),
                BigInt(
                    "5591557118325006940652332791312874698324071372762903093203759620236776485604",
                ),
            ],
            pubkeyG2: [
                [
    ...
    describe("TestEncodeChurnRegistrationParams", () => {
        test("test registration with churn params encoding", () => {
            const registrationParams: PubkeyRegistrationParams =
            ...
            const signatureWithSaltAndExpiry: SignatureWithSaltAndExpiry = {
                signature:
                    "0xd547fa0126f97d1752a3b3103c495961a4a6a7a5386feb32ac514289c578db5a0d64bfa34855c39d78cce241a9c73d5cae47dee02c691fd5320fdef4ad3e1f8e",
    ...
    describe("ChainWriter", async () => {
        ...
        const { clients, addresses: contractAddrs } =
            await buildClients(endpoint);
        const client0 = clients[0];
        ...
        test("update operator details", async () => {
            ...
            const receipt =
                await client0.elWriter.updateOperatorDetails(operatorModified);
            expect(receipt.status).toBe(1n);
        });
    ...
    describe("TestSetOperatorPISplit", async () => {
        ...
        test("should update operator PI split correctly", async () => {
            ...
            const receiptDelay: TransactionReceipt =
                await setTestRewardsCoordinatorActivationDelay(
                    endpoint,
                    testUtils.ANVIL_FIRST_PRIVATE_KEY,
                    0n,
                );
            ...
            const initialSplit: bigint = await rewardsCoordinator.methods
                .getOperatorPISplit(operatorAddr)
                .call();
            ...
            const tx: TransactionReceipt =
                await client0.elWriter.setOperatorPiSplit(
                    operatorAddr,
                    newSplit,
                );
            ...
            const updatedSplit: bigint = await rewardsCoordinator.methods
                .getOperatorPISplit(operatorAddr)
                .call();
    ...
    describe("TestSetOperatorAVSSplit", async () => {
        ...
        test("should update operator AVS split correctly", async () => {
            ...
            const initialSplit: bigint =
                await client0.elWriter.rewardsCoordinator.methods
                    .getOperatorAVSSplit(operatorAddr, avsAddr)
                    .call();
            ...
            const receipt = await sendContractCall({
                ...
                params: [operatorAddr, avsAddr, newSplit],
    ...
    describe("TestSetOperatorSetSplit", async () => {
        ...
        test("should register operator in operator set and update split", async () => {
            const { rewardsCoordinator, registryCoordinator } =
                client0.elWriter;
            ...
            const receiptDelay: TransactionReceipt =
                await rewardsCoordinator.methods
                    .setActivationDelay(0n)
                    .send({ from });
            expect(receiptDelay.status).toEqual(1n);
            ...
            // 6. Set new split
            const newSplit = initialSplit + 1n;
            const receiptSet = await rewardsCoordinator.methods
                .setOperatorSetSplit(
                    operatorAddr,
                    { avs: avsAddr, id: operatorSetId },
                    newSplit,
            ...
            const updatedSplit: bigint = await rewardsCoordinator.methods
                .getOperatorSetSplit(operatorAddr, {
                    avs: avsAddr,
                    id: operatorSetId,
                })
                .call();
    ...
    describe("TestSetAndRemovePermission", async () => {
        ...
        test("set permission to account", async () => {
            ...
            const canCall = await client0.elReader.canCall(
                accountAddress,
                appointeeAddress,
                target,
                selector,
    ...
    describe("TestModifyAllocations", async () => {
        ...
        test("should enforce allocation delay before modifying allocations", async () => {
            ...
            const delay = 1n;
            const receipt1 = await client0.elWriter.setAllocationDelay(
                operatorAddr,
                delay,
            );
            expect(receipt1.status).toBe(1n);
            ...
            await testUtils.advanceChainByNBlocksExecInContainer(
            ...
            let allocationInfo = await client0.elReader.getAllocationInfo(
                operatorAddr,
                strategyAddr,
            );
            ...
            const allocationDelay =
                await client0.elReader.getAllocationDelay(operatorAddr);
            await testUtils.advanceChainByNBlocksExecInContainer(
                Number(allocationDelay),
                container,
            );

            // Verify allocation updated
            allocationInfo = await client0.elReader.getAllocationInfo(
                operatorAddr,
                strategyAddr,
            );
    ...
    describe(
        ...
        async () => {
            ...
            test("should clear deallocation queue correctly", async () => {
                ...
                // Initialize allocation delay
                let receipt = await client0.elWriter.setAllocationDelay(
                    operatorAddr,
                    delay,
                );
                expect(receipt.status).toBe(1n);
                ...
                receipt = await client0.elWriter.clearDeallocationQueue(
                    operatorAddr,
                    strategies,
                    numsToClear,
                );
                ...
                await expect(
                    client0.elWriter.clearDeallocationQueue(
                        operatorAddr,
                        strategies,
                        invalidNumsToClear,
                    ),
    ...
    describe("TestAddAndRemovePendingAdmin", async () => {
        ...
        test("should add pending admin", async () => {
            ...
            const isPendingAdmin = await elReader.isPendingAdmin(
                operatorAddr,
                pendingAdmin,
            );
    ...
    describe("TestAcceptAdmin", async () => {
        ...
        let accountChainWriter: Clients = await buildSingleClient(
            testUtils.ANVIL_FIRST_PRIVATE_KEY,
            endpoint,
        );
        let adminChainWriter: Clients = await buildSingleClient(
        ...
        beforeAll(async () => {
            ...
            const receipt =
                await accountChainWriter.elWriter.addPendingAdmin(addRequest);
            expect(receipt.status).toBe(1n);
        });
        ...
        test("should accept admin", async () => {
            ...
            const receipt =
                await adminChainWriter.elWriter.acceptAdmin(acceptRequest);
            expect(receipt.status).toBe(1n);

            const isAdmin = await elReader.isAdmin(
                accountAddr,
                pendingAdminAddr,
            );
    ...
    describe("TestRemoveAdmin", async () => {
        ...
        let admin1ChainWriter: Clients = await buildSingleClient(
            testUtils.ANVIL_SECOND_PRIVATE_KEY,
            endpoint,
        );
        let admin2ChainWriter: Clients = await buildSingleClient(
            testUtils.ANVIL_THIRD_PRIVATE_KEY,
            endpoint,
        );
        ...
        beforeAll(async () => {
            ...
            let receipt =
                await accountChainWriter.elWriter.addPendingAdmin(
                    addAdmin1Request,
                );
            expect(receipt.status).toBe(1n);

            receipt =
                await admin1ChainWriter.elWriter.acceptAdmin(
                    acceptAdminRequest,
                );
            expect(receipt.status).toBe(1n);
            ...
            receipt =
                await admin1ChainWriter.elWriter.addPendingAdmin(
                    addAdmin2Request,
                );
            expect(receipt.status).toBe(1n);

            receipt =
                await admin2ChainWriter.elWriter.acceptAdmin(
                    acceptAdminRequest,
                );
            expect(receipt.status).toBe(1n);
        });
        ...
        test("should remove admin 2", async () => {
            ...
            const receipt =
                await admin1ChainWriter.elWriter.removeAdmin(
                    removeAdminRequest,
                );
            expect(receipt.status).toBe(1n);
    ...
    describe(
        ...
        async () => {
            ...
            test("should process a claim successfully", async () => {
                ...
                const claim: ClaimCheckParams = await newTestClaim(
                    elReader,
                    endpoint,
                    cumulativeEarnings,
                    testUtils.ANVIL_FIRST_PRIVATE_KEY,
    ...
    describe(
        ...
        async () => {
            ...
            test("should process multiple claims successfully", async () => {
                ...
                let receipt = await setTestRewardsCoordinatorActivationDelay(
                    endpoint,
                    privateKeyHex,
                    activationDelay,
                );
                ...
                // Generate 2 claims
                const claim1 = await newTestClaim(
                    elReader,
                    endpoint,
                    cumulativeEarnings1,
                    privateKeyHex,
                );
                const claim2 = await newTestClaim(
                    elReader,
                    endpoint,
                    cumulativeEarnings2,
                    privateKeyHex,
    ...
    async function setTestRewardsCoordinatorActivationDelay(
        ...
        const client: Clients = await buildSingleClient(
            privateKeyHex,
            httpEndpoint,
        );
    ...
    async function newTestClaim(
        ...
        const clients: Clients = await buildSingleClient(
            privateKeyHex,
            rpcEndpoint,
        );
        ...
        const contractAddrs =
            await testUtils.getContractAddressesFromContractRegistry(
                rpcEndpoint,
            );
        ...
        const tokenAddr: string = await contractStrategy.methods
            .underlyingToken({})
            .call();
        ...
        const cumulativeEarningsBytes = tokenLeaf.cumulativeEarnings
            .toString(16)
            .padStart(64, "0");
        ...
        const currRewardsCalculationEndTimestamp =
            await elReader.currRewardsCalculationEndTimestamp();
        ...
        const submitRootTx = await sendContractCall({
            contract: clients.elWriter.rewardsCoordinator,
            method: "submitRoot",
            params: [root, currRewardsCalculationEndTimestamp + 1n],
            pkWallet,
            web3,
        });
